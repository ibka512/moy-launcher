...
class MoyFourPanelView(
    ...
    private fun titleBlock(): View = LinearLayout(context).apply {
        ...
        addView(
            label("MOY", 15f, Color.rgb(153, 196, 255), bold = true).apply {
                letterSpacing = 0.18f
            },
        )
        addView(
            label("我的四分屏", 30f, Color.WHITE, bold = true).apply {
                setPadding(0, dp(6), 0, 0)
            },
        )
        addView(
            label("先把手机里最重要的四件事放在眼前", 14f, Color.rgb(201, 207, 226)).apply {
